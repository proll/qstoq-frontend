qst.InfoPageView = qst.PageView.extend({
	...
	changeSection: function(model, value) {
		if(this.$cont) {
			this.$cont
				.removeAttr('class')
				.addClass('page-info_'+value)
		}
	},

	addMenu: function (menu_model) {
		this.$el.find('.info__menu-row').html(menu_model.view.$el);
	},

	addInfo: function (info_model) {
		this.$el.find('.info__section-row').html(info_model.view.$el);
	},
});